...
import logging
from drive.models import create_indices, FileIndices
from pandas import DataFrame, read_csv
from drive.helper_funcs import split_target_string
from typing import TypeVar, Optional, Callable
from dataclasses import dataclass, field
from typing import Iterator
from drive.models.types import Genes
...
class IbdFilter:
    ...
    def load_file(
        ...
        """
        logger.debug(f"Reading in the ibd input file at {ibd_file}")
    ...
    def _generate_map(self, chunk_data: DataFrame) -> None:
        ...
        logger.debug(f"identified {len(haplotypes)} haplotypes in this chunk.")
...
def pandas_ibd_filter(
    ...
    indices = create_indices(ibd_format)

    target_gene = split_target_string(region)

    filter_obj: IbdFilter = IbdFilter.load_file(
        ibd_file, indices, target_gene, chunksize
    )

    filter_obj.set_filter(overlap)

    filter_obj.preprocess(min_cm, None)
